package frc.robot.constants;

public class CanIdConstants {

  /** Constants for swerve */
  public static class Swerve {
    public static final int FRONT_LEFT_DRIVE = 1;
    public static final int FRONT_LEFT_ENCODER = 2;
    public static final int FRONT_LEFT_STEER = 3;

    public static final int FRONT_RIGHT_DRIVE = 4;
    public static final int FRONT_RIGHT_ENCODER = 5;
    public static final int FRONT_RIGHT_STEER = 6;

    public static final int BACK_LEFT_DRIVE = 7;
    public static final int BACK_LEFT_ENCODER = 8;
    public static final int BACK_LEFT_STEER = 9;

    public static final int BACK_RIGHT_DRIVE = 10;
    public static final int BACK_RIGHT_ENCODER = 11;
    public static final int BACK_RIGHT_STEER = 12;

    public static final int PIGEON = 13;
  }

  /** Constants for intake */
  public static class Intake {
    public static final int INTAKE_MOTOR = 20;
    public static final int INTAKE_FOLLOWER_MOTOR = 21;
    public static final int DEPLOYMENT_MOTOR = 22;
  }

  /** Constants for indexer */
  public static class Indexer {
    public static final int INDEXER_MOTOR = 30;
  }

  /** Constants for shooter */
  public static class Shooter {
    public static final int SHOOTER_FLYWHEEL_1 = 40;
    public static final int SHOOTER_FLYWHEEL_2 = 41;
    public static final int SHOOTER_FLYWHEEL_3 = 42;
    public static final int SHOOTER_FLYWHEEL_4 = 43;
    public static final int HOOD_MOTOR = 44;
  }

  /** Constants for climber */
  public static class Climber {
    public static final int CLIMBER_MOTOR = 50;
  }

  /** Constants for feeder */
  public static class Feeder {
    public static final int FEEDER_MOTOR = 60;
    public static final int FEEDER_FOLLOWER_MOTOR = 61;
  }
}
